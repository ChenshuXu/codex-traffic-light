...
On first launch after this upgrade, the app imports existing unread thread IDs from VS Code's local state database. It then connects to the current user's `~/.codex/ipc/ipc.sock` and listens only for `thread-read-state-changed` events from Codex Desktop and VS Code. The app stores the resulting thread ID set in its own UserDefaults and never writes to Codex's task database or conversation records.
